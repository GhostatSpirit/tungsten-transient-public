...
static bool evalTransientBeam1D(const Transient::TransientPhotonBeam &beam, PathSampleGenerator &sampler, const Ray &ray, const Medium *medium,
        const Vec3pf *bounds, const PhotonMapSettings& settings, float tMin, float tMax, float radius, float timeWidth, float camTimeTraveled, Vec3f &beamEstimate)
{
    float invSinTheta, t, s;
    if (intersectBeam1D(beam, ray, bounds, tMin, tMax, radius, invSinTheta, t, s)) {
        Vec3f hitPoint = ray.pos() + ray.dir()*t;
        ...
        float timeBeg = settings.transientTimeCenter - timeWidth * 0.5f;
        float timeEnd = settings.transientTimeCenter + timeWidth * 0.5f;
        if (pathTime < timeBeg || pathTime > timeEnd)
        {
            return false;
        }
        timeFactor = 1.0f / timeWidth;

        Ray mediumQuery(ray);
        mediumQuery.setFarT(t);
        beamEstimate += medium->sigmaT(hitPoint)*invSinTheta/(2.0f*radius)
                *medium->phaseFunction(hitPoint)->eval(beam.dir, -ray.dir())
                *medium->transmittance(sampler, mediumQuery, true, false)*beam.power*timeFactor;
...
static const float timeInVolume(const Medium *medium, const Vec3f &p, const Vec3f &a, const Vec3f &b, const Vec3f &c, const Vec3f& uvw)
{
    Vec3f p0 = p;
    Vec3f p1 = p0 + a * uvw.x();
    Vec3f p2 = p1 + b * uvw.y();
                                ...
                                PathInfo &pInfo, Vec3f &totalEstimate)
{
    ...
    if (intersectVolume(ray, tMin, tMax, p.p, p.a, p.b, p.c, o, d, minT, maxT))
    {
        ...
        float minTimeTraveled = timeInVolume(medium, p.p, p.a, p.b, p.c, minUvw);
        float maxTimeTraveled = timeInVolume(medium, p.p, p.a, p.b, p.c, maxUvw);
        ...
        float minRatio = (transMinTime - minTimeTraveled) / (maxTimeTraveled - minTimeTraveled);
        float maxRatio = (transMaxTime - minTimeTraveled) / (maxTimeTraveled - minTimeTraveled);
        ...
        PhotonMapSettings::VolumePhotonType photonType, Ray &depthRay, bool useFrustumGrid)
{
    ...
    while ((medium || didHit) && bounce < _settings.maxBounces) {
        ...
        if (medium) {
            if (bounce > 1 || !useFrustumGrid) {
                ...
                auto beamContribution = [&](uint32 photonIndex, const Vec3pf *bounds, float tMin, float tMax) {
                    ...
                    if constexpr (isTransient)
                        evalTransientBeam1D(beam, sampler, ray, medium, bounds, _settings, tMin, tMax, volumeGatherRadius, beamTimeWidth, timeTraveled, estimate);
                    else
        ...
        if (!includeSurfaces && !bsdf.lobes().isForward())
            break;
        ...
        PathPhotonRange &pathRange, PathSampleGenerator &sampler)
{
    ...
    bool traceHigherOrder = numGhostBounces > 0;
